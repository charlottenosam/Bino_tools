...
# =====================================================================
# Combine directory
combine_dir = os.environ['BINO_DIR']+'SAO-9/combined_fluxcalib/combine_%i_frames_seeing<%.2farcsec_%s_parallel' % \
...
hdu_flux = hdu0.copy()
hdu_err  = hdu_err0.copy()

# =====================================================================
# Combine each slit
def combine_slit(slit_i):
    
    slit = hdu_flux[slit_i]

    if type(slit.data) == np.ndarray:
        ...
        return flux_combined_clipped, err_combined_clipped
    
combined_slits = Parallel(n_jobs=num_cores-2)(delayed(combine_slit)(i) for i in range(nslits))



# # Do flux calib
# hdu_flux[slit_i].data = flux_combined_clipped 
# hdu_err[slit_i].data  = err_combined_clipped

# hdu_flux[slit_i].header['EXPTIME'] = exptime * n_ob
# hdu_err[slit_i].header['EXPTIME']  = exptime * n_ob

# hdu_flux.writeto(flux_combined_file, overwrite=True)
# hdu_err.writeto(err_combined_file, overwrite=True)
